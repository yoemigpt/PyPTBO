...
class Perturbation(ABC):
    ...
    @abstractmethod
    def pdf(self, theta: Tensor, eta: Tensor) -> Tensor:
        pass
...
class NormalPerturbation(Perturbation):
    ...
    def pdf(self, theta: Tensor, eta: Tensor) -> Tensor:
        return exp(-0.5 * ((eta - theta) / self.sigma) ** 2) / (self.sigma * sqrt(2 * pi))
...
class GumbelPerturbation(Perturbation):
    ...
    def sample(self, theta: Tensor, n_samples: int = 1) -> Tensor:
        device = theta.device
        return theta + self.alpha * rand(n_samples, *theta.size(), device=device)
